...
public:  // ***************************************************************************************
	...
	// NGMP: Minimal 2D draw primitives backing GOPluginHostAPI::drawText2D/drawRect2D (see
	// PluginABI.h), called from within GOPluginManager::DispatchDrawOverlay(), i.e. only ever
	// during postWindowDraw() where screen-space 2D drawing is already set up. Only called on
	// TheInGameUI (not virtual dispatch relevant), so implemented once here rather than per
