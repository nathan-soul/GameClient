...
void InGameUI::postWindowDraw()
{
	...
	// TheSuperHackers @feature 03/08/2026 Live streaming / live-observer status banner.
	drawLiveStatus();
...
void InGameUI::drawObserverStats(Int & x, Int & y)
{
	...
	Int baseX = (screenW - bgW) / 2;	// center overlay horizontally
...
//-------------------------------------------------------------------------------------------------
// TheSuperHackers @feature 03/08/2026 Live streaming / live-observer status banner, top-centre.
// Called from InGameUI::draw() so it is visible in every session type: for the streamer (who is a
// normal player) as much as for an observer watching a relayed game.
//-------------------------------------------------------------------------------------------------
void InGameUI::drawLiveStatus()
{
	...
	if (TheLiveStreamer && (TheLiveStreamer->isStreaming() || TheLiveStreamer->isBackup()))
	{
		...
		if (streamStatus)
		{
			...
			statusText.translate(AsciiString(TheLiveStreamer->isStreaming() ? "STREAMING" : "BACKUP"));
			streamStatus->setText(statusText);

			Int statusX = (TheDisplay->getWidth() - streamStatus->getWidth()) / 2;
	...
	if (TheLiveObserver && TheLiveObserver->isConnected())
	{
		...
		if (observerStatus)
		{
			...
			if (!label.isEmpty())
			{
				...
				Int statusX = (TheDisplay->getWidth() - observerStatus->getWidth()) / 2;
